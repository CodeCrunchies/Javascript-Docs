...
navbarLinks.forEach((navbarLink) => {
  navbarLink.addEventListener("click", (e) => {
    ...
    setTimeout(
      () =>
        EPPZScrollTo.scrollVerticalToElementById(
          navbarLink.getAttribute("href").slice(1),
          60
        ),
  ...
  if (viewingSection) {
    Array.from(navbarLinks).forEach((navbarLink) => {
      if (
        ...
        `#${viewingSection.getAttribute("id")}`
      ) {
...
var EPPZScrollTo = {
  ...
  documentHeight: function () {
    return document.height !== undefined
      ? document.height
  ...
  scrollVerticalTickToPosition: function (currentPosition, targetPosition) {
    ...
    // Filtered position.
    currentPosition =
      parseFloat(currentPosition) * (1.0 - filter) +
  ...
  scrollVerticalToElementById: function (id, padding) {
    ...
    var targetPosition =
      ...
      this.elementVerticalClientPositionById(id) -
      padding;
